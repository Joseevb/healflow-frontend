...
export class UserSyncService {
  ...
  ): Effect.Effect<Array<string>, UserSyncError> {
    ...
    return Effect.gen(function* () {
      ...
      const apiKey = yield* apiKeyConfig
        ...
        .pipe(
          Effect.mapError(
            (err) =>
              new UserSyncError({ message: `API key error: ${err._tag}` }),
          ),
        )
      ...
      const headerName = yield* apiKeyConfig
        ...
        .pipe(
          Effect.mapError(
            (err) =>
              new UserSyncError({ message: `API header error: ${err._tag}` }),
          ),
        )
      ...
      const result = yield* Effect.tryPromise({
        try: async () => {
          ...
          const message = err instanceof Error ? err.message : String(err)
          return new UserSyncError({
            message: `Validation request failed: ${message}`,
          })
        },
      })
      ...
      if (result.response.status === 400 && result.error) {
        ...
        console.log(
          `[UserSync] Found ${invalidIds.length} invalid user(s):`,
          invalidIds,
        )
        return invalidIds
      }
  ...
  deleteUser(userId: string): Effect.Effect<void, UserSyncError> {
    return Effect.tryPromise({
      try: async () => {
        ...
        const message = err instanceof Error ? err.message : String(err)
        return new UserSyncError({
          message: `Failed to delete user ${userId}: ${message}`,
        })
      },
    })
  }
  ...
  sync(): Effect.Effect<{ validated: number; deleted: number }, UserSyncError> {
    return Effect.gen(
      function* (this: UserSyncService) {
        ...
        console.log(
          `[UserSync] Found ${userIds.length} user(s) in auth database`,
        )
        ...
        if (invalidUserIds.length > 0) {
          console.log(
            `[UserSync] Removing ${invalidUserIds.length} stale user(s)...`,
          )
