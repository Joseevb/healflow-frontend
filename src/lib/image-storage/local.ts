    ...
    Effect.gen(function* () {
      ...
      yield* Effect.tryPromise({
        try: () => Bun.$`mkdir -p ${dir}`.text().then(() => undefined),
          ...
          new StorageError({ message: `Failed to create directory: ${dir}` }),
      })
      ...
      const arrayBuffer = yield* Effect.tryPromise({
        try: () => file.arrayBuffer(),
          ...
          new StorageError({ message: `Failed to read file: ${file.name}` }),
      })
      ...
      yield* Effect.tryPromise({
        try: async () => {
          ...
          new StorageError({ message: `Failed to write file: ${filepath}` }),
      })
  ...
  delete: (url) => {
    ...
    return Effect.tryPromise({
      ...
      try: () => Bun.file(filepath).delete(),
        ...
        new StorageError({ message: `Failed to delete file: ${filepath}` }),
    }).pipe(
