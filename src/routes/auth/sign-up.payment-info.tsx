...
const searchParams = z.object({
  error: z
    .enum(['provisioning_failed', 'cancelled', 'unknown_error'])
    .optional(),
})
...
export default function RouteComponent() {
  ...
  const handleSubscribe = async (planId: 'monthly' | 'yearly') => {
    ...
    const { error } = await authClient.subscription.upgrade({
      ...
      cancelUrl: createUrl('/auth/sign-up/payment-info', {
        error: 'cancelled',
      }),
    })
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto">
      ...
      <p className="text-center text-sm text-muted-foreground">
        ...
        <span className="font-medium text-foreground">Stripe</span>. Your card
