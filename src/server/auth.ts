 ...
 */
export const checkIsNewUser = createServerFn({ method: 'GET' }).handler(
  async () => {
    ...
    const { data: validationResult, error: validationError } = await attempt(
      async () => {
...
export const getServerSession = createServerFn({ method: 'GET' }).handler(
  async () => {
    ...
    if (session) {
      console.log('[getServerSession] User authenticated:', {
 ...
 */
export const getJwtToken = createServerFn({ method: 'GET' }).handler(
  async () => {
    const headers = getRequestHeaders()
